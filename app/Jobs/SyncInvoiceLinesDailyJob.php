...
class SyncInvoiceLinesDailyJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $year = now()->year;
            $month = now()->month;
            Log::info("Filtering records for year: {$year}, month: {$month}");
            ...
            Log::info("Found {$recordCount} records matching the criteria");

            if ($recordCount === 0) {
                // Try with less restrictive criteria as a test
                $testCount = InvReceipt::count();
                Log::info("Total records in InvReceipt table: {$testCount}");

                $confirmedCount = InvReceipt::where('is_confirmed', 'Yes')->count();
                Log::info("Records with is_confirmed='Yes': {$confirmedCount}");
                ...
                Log::info("Sample records with is_confirmed='Yes' AND empty inv_doc_no: " . json_encode($testRecords));

                $emptyInvDocCount = InvReceipt::where('inv_doc_no', '')->count();
                Log::info("Records with empty inv_doc_no: {$emptyInvDocCount}");
                ...
                $dateCount = InvReceipt::whereYear('actual_receipt_date', $year)
                    ->whereMonth('actual_receipt_date', $month)
                    ->count();
                Log::info("Records for current year/month: {$dateCount}");

                Log::warning("No records match all criteria together. Check field values in the database.");
            }
            ...
            foreach ($sqlsrvData as $data) {
                ...
                Log::debug("Processing record: po_no={$data->po_no}, receipt_no={$data->receipt_no}, receipt_line={$data->receipt_line}, is_confirmed={$data->is_confirmed}, inv_doc_no=" . ($data->inv_doc_no ?? 'NULL'));

                // Check if data->po_no is not empty
                if (empty($data->po_no) || empty($data->receipt_no) || empty($data->receipt_line)) {
                    Log::warning("Skipping record with empty key fields: po_no={$data->po_no}, receipt_no={$data->receipt_no}, receipt_line={$data->receipt_line}");
                    continue;
                }
                ...
                try {
                    ...
                    DB::rollBack();
                    Log::error("Error processing record: po_no={$data->po_no}, receipt_no={$data->receipt_no}, receipt_line={$data->receipt_line}. Error: " . $e->getMessage());
                }
            }
            ...
            Log::error('An error occurred during daily synchronization job: ' . $e->getMessage(), [
